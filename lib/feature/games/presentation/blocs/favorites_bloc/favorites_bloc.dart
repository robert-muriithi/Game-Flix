...
class FavoritesBloc extends Bloc<FavoritesEvent, FavoritesState> {
  ...
  FavoritesBloc(this.getGameFromFavorites, this.addGameToFavoritesUseCase, this.removeGameFromFavoritesUseCase)
      : super(FavoritesInitial()) {
    on<FavoritesEvent>(favoritesEventsObserver);
  }

  FutureOr<void> favoritesEventsObserver(FavoritesEvent event, Emitter<FavoritesState> emit) async {
     if (event is GetFavoriteEvent) {
      emit(FavoritesLoading());
      final result = await getGameFromFavorites(event.id);
      result.fold(
        (failure) => emit(FavoritesError(message: mapFailureToMessage(failure))),
